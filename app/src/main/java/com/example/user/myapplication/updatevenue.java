...
public class updatevenue extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        et_venuename.setText(getIntent().getStringExtra("venuename"));
        et_venue_loc.setText(getIntent().getStringExtra("venueloc"));
        et_price.setText(getIntent().getStringExtra("venueprice"));
    ...
    public void update_data(View view) {
        ...
        if (venuename.length()< 2 || venue_loc.length() < 2 || price == 0 || capacity == 0) {
            Toast.makeText(updatevenue.this, "Enter values", Toast.LENGTH_SHORT).show();
            ...
            database.getReference().child("venue").child(venuename + venue_loc).setValue(data).addOnCompleteListener(new OnCompleteListener<Void>() {
                ...
                public void onComplete(@NonNull Task<Void> task) {
                    ...
                    {
                        startActivity( new Intent(updatevenue.this , stp2admin.class));

                        Toast.makeText(updatevenue.this , "Data updated" , Toast.LENGTH_SHORT).show();
                    }
                    else {

                        Toast.makeText(updatevenue.this , "Error" , Toast.LENGTH_SHORT).show();
                    }
                }
            });
